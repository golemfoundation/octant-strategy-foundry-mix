...
contract LiquidityStrategyTest is BaseTest {
    // =========== Constants ===========
    uint256 constant INITIAL_DEPOSIT = 1e7;
    ...
    function testFreeFunds() public {
        ...
        console2.log("Position amount0", posAmount0);
        console2.log("Position amount1", posAmount1);
        ...
        console2.log("Final liquidity", uint256(finalLiquidity));
    ...
    function testHarvestAndReport() public {
        ...
        assertTrue(module.harvestTrigger(), "Harvest should be triggered");
        ...
        vm.prank(keeper);
        module.tend();
        // emit log_named_uint("Module balance", asset.balanceOf(address(module)));
    }
    ...
    function testHarvestTrigger() public {
        ...
        // Deposit funds
        _deposit(INITIAL_DEPOSIT);
        assertTrue(module.harvestTrigger(), "Should trigger after deposit");
    ...
    /// @param tokenId Position NFT ID to view
    function _viewPosition(uint256 tokenId) internal view {
        (,, address token0, address token1, uint24 fee, int24 tickLower, int24 tickUpper, uint128 liquidity,,,,) =
            INonfungiblePositionManager(UNISWAP_V3_POSITION_MANAGER).positions(tokenId);

        console2.log("Position token0", token0);
        console2.log("Position token1", token1);
        console2.log("Position fee", fee);
        console2.log("Position tickLower", tickLower);
        console2.log("Position tickUpper", tickUpper);
